...
public class SwiftFlowInstance: ObservableObject {
    ...
    public func fitView<T: Equatable & Sendable>(
        ...
        let bounds = computeBounds(nodes: targetNodes, nodeSizes: nodeSizes, includeHidden: options.includeHiddenNodes)
        ...
        let zoom = max(min(min(effectiveWidth / cw, effectiveHeight / ch), options.maxZoom), options.minZoom)

        let vp = Viewport(x: -(bounds.minX * zoom) + padding, y: -(bounds.minY * zoom) + padding, zoom: zoom)
