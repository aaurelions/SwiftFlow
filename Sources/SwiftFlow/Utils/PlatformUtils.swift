...
// MARK: - macOS Scroll Wheel Handling
      ...
      override func viewDidMoveToWindow() {
        ...
        eventMonitor = NSEvent.addLocalMonitorForEvents(matching: .scrollWheel) {
          ...
          guard event.phase != [] || !event.hasPreciseScrollingDeltas else {
            ...
            guard abs(delta.width) > 0.5 || abs(delta.height) > 0.5 else { return event }
            onScroll(delta, location)
            return nil  // consume
          }
          let delta = CGSize(width: event.scrollingDeltaX, height: event.scrollingDeltaY)
          guard abs(delta.width) > 0.1 || abs(delta.height) > 0.1 else { return event }
          if event.hasPreciseScrollingDeltas {
            onScroll(delta, location)
          } else {
            onScroll(CGSize(width: delta.width * 10, height: delta.height * 10), location)
          }
          return nil  // consume
        }
      }
  ...
  extension View {
    func onScrollWheel(_ handler: @escaping (CGSize, CGPoint) -> Void) -> some View {
