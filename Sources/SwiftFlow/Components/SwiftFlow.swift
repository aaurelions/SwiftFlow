...
>: View {
    ...
    public var body: some View {
        GeometryReader { proxy in
            ...
            #if canImport(AppKit)
                .onKeyDown { event in
                    guard elementsSelectable else { return false }
        ...
        .background(
            theme.canvasBackgroundColor == .clear
                ? Color.swiftFlowBackground : theme.canvasBackgroundColor
        )
        .clipped()
        .environmentObject(flowState)
        .environment(\.swiftFlowInstance, swiftFlowInstance)
        .environment(\.nodesInitialized, !nodes.isEmpty && nodes.allSatisfy {
            nodeSizes[$0.id] != nil
        })
        .onAppear {
            syncFlowState()
    ...
    private var interactionLayer: some View {
        Color.clear
            ...
            .flowCursor(.grab)
            .onTapGesture {
                guard elementsSelectable else { return }
                onPaneClick?()
                deselectAll()
            }
            ...
            .simultaneousGesture(zoomOnPinch ? magnificationGesture : nil)
            #if canImport(AppKit)
            .if(zoomOnScroll) { view in
        ...
        ForEach(sortedNodes) { node in
            if !node.hidden && !isParentCollapsed(node) && (node.type != "group" || node.expanded) {
                ...
                if isNodeVisible(absPos, size: nodeSize) {
                    ...
                    .fixedSize()
                    .contentShape(Rectangle())
                    .gesture(nodesDraggable && node.draggable ? nodeDragGesture(node: node) : nil)
                    ...
                    .simultaneousGesture(
                        TapGesture(count: 1).onEnded {
                            guard elementsSelectable && node.selectable else { return }
                            onNodeClick?(node)
        ...
        ForEach(sortedEdges) { edge in
                    ...
                    absolutePositions: absPositions)
            {
                ...
                let pathResult = getEdgePathResult(
                    ...
                    targetY: tPos.y)
                let color = edge.style?.strokeColor ?? (edge.selected ? theme.edgeSelectedColor : theme.edgeColor)
                let width = edge.style?.strokeWidth ?? (edge.selected ? theme.edgeSelectedWidth : theme.edgeWidth)
                ...
                ZStack {
                    ...
                    if edge.reconnectable && edge.selected && nodesConnectable {
            ...
            .gesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { value in
                        ...
                        let anchorType = lookupHandleType(nodeId: draftingSourceInfo?.nodeId ?? "", handleId: draftingSourceInfo?.handleId ?? "") ?? .source
                        var snappedTarget: (nodeId: String, handleId: String, handleType: HandleType?)?
                        if let snapped = findClosestHandle(to: rawPos, threshold: 40, preferType: anchorType == .source ? .target : .source),
                            snapped.nodeId != excludeNode
                        {
                            draftingConnectionCurrent = lookupHandlePos(nodeId: snapped.nodeId, handleId: snapped.handleId, type: snapped.handleType)
    ...
    private func connectionDragGesture(nodeId: String, handleId: String, handleType: HandleType? = nil, handlePos: CGPoint)
        -> some Gesture
    {
        DragGesture(minimumDistance: 0)
            .onChanged { value in
                if draftingConnectionStart == nil {
                    ...
                    let resolvedHandleType = handleType ?? lookupHandleType(nodeId: nodeId, handleId: handleId) ?? .source
                    onConnectStart?(
                ...
                let sourceType = handleType ?? lookupHandleType(nodeId: nodeId, handleId: handleId) ?? .source
                var snappedTarget: (nodeId: String, handleId: String, handleType: HandleType?)?
                if let snapped = findClosestHandle(to: rawPos, threshold: 40, preferType: sourceType == .source ? .target : .source),
    ...
    private func finishDraftingConnection() {
        ...
        guard let target = findClosestHandle(to: endPos, threshold: 40, preferType: sourceType == .source ? .target : .source),
        ...
        if connectionMode == .strict {
            ...
            guard resolvedSourceType == .source && targetType == .target else {
    ...
    private func getHandlePos(
        ...
        let typedKey = HandlePositionPreferenceKey.makeKey(nodeId: nodeId, handleId: handleId ?? "", type: handleType)
    ...
    private func lookupHandlePos(nodeId: String, handleId: String, type: HandleType? = nil) -> CGPoint? {
        if let type {
    ...
    private func findClosestHandle(to point: CGPoint, threshold: CGFloat, preferType: HandleType? = nil) -> (
