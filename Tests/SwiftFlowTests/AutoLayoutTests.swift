...
struct AutoLayoutTests {
    ...
    @Test func treeLayoutTopToBottom() {
        let changes = computeAutoLayout(nodes: nodes, edges: edges, algorithm: .tree(direction: .topToBottom))
    ...
    @Test func treeLayoutLeftToRight() {
        let changes = computeAutoLayout(nodes: nodes, edges: edges, algorithm: .tree(direction: .leftToRight))
        let result = applyNodeChanges(changes, nodes: nodes)
        let n1 = result.first { $0.id == "1" }!
        let n2 = result.first { $0.id == "2" }!
    ...
    @Test func treeLayoutBottomToTop() {
        let changes = computeAutoLayout(nodes: nodes, edges: edges, algorithm: .tree(direction: .bottomToTop))
        let result = applyNodeChanges(changes, nodes: nodes)
        let n1 = result.first { $0.id == "1" }!
        let n2 = result.first { $0.id == "2" }!
        #expect(n1.position.y > n2.position.y)
    ...
    @Test func treeLayoutRightToLeft() {
        let changes = computeAutoLayout(nodes: nodes, edges: edges, algorithm: .tree(direction: .rightToLeft))
        let result = applyNodeChanges(changes, nodes: nodes)
        let n1 = result.first { $0.id == "1" }!
        let n2 = result.first { $0.id == "2" }!
        #expect(n1.position.x > n2.position.x)
